...
Se implementó un **motor de scoring por aritmética ponderada** basado en tags/preferencias. Cada tag en el sistema tiene un `peso_matching` configurable y un flag `es_filtro_absoluto`. El algoritmo reemplaza el anterior Jaccard index simple que estaba en `mascotaService.calcularCompatibilidad`.
...
| `src/controllers/matchController.js` | Handlers Express para los 3 endpoints |
| `src/routes/matchRoutes.js` | Definición de rutas + documentación Swagger |
| `src/tests/matchService.test.js` | Tests unitarios (9 casos) |
...
|--------|------|------|-----|-------------|
| `POST` | `/api/match/calcular/:id_adoptante` | JWT | admin | Ejecuta el motor de matching y persiste resultados |
