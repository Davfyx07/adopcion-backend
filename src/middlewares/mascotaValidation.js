...
const validateCreateMascota = (req, res, next) => {
    ...
    if (!tagsIds || !Array.isArray(tagsIds)) {
        ...
        if (invalidTags.length > 0) {
            errors.push({ field: 'tagsIds', message: 'Todos los tagsIds deben ser UUID válidos.' });
        }
    }
...
const validateUUIDParam = (req, res, next) => {
    ...
    if (!id || !UUID_REGEX.test(id)) {
        return res.status(400).json({ success: false, message: 'ID de mascota inválido (debe ser un UUID válido).' });
    }
    return next();
};
